...
const initializeMentorshipPackages = async () => {
  try {
    ...
    mentorshipPackages = mentorshipPlans.map(plan => {
      ...
      const memberPrice = plan.pricing?.oneTime?.memberPrice || Math.round(basePrice * 0.8);
      const calculatedSavings = plan.pricing?.oneTime?.savings ||
        plan.pricing?.monthly?.savings ||
        Math.max(0, basePrice - memberPrice);
